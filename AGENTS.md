...
* **Input**
  ...
  * `query` *(string, required)* — short, specific semantic query (≤1000 chars).
  * `type` *(string, optional)* — scope by type (e.g., `"note"`, `"decision"`, `"todo"`).
  * `tags` *(string\[], optional)* — use normalized tags like `project:alpha`, `topic:etl`.
...
{"jsonrpc":"2.0","id":23,"method":"tools/call","params":{
  ...
  "arguments":{
    "type":"runbook",
    "content":{"title":"Service-X Rollback v2","steps":[/* concise */]},
    "source":"chat://claude/session-2025-09-17",
    "tags":["project:service-x","topic:release","status:accepted"],
    "confidence":0.95
  }
...
```json
{"jsonrpc":"2.0","id":"save-1","method":"tools/call","params":{
  "name":"memory_create",
  "arguments":{
    "type":"note",
...
```json
{"jsonrpc":"2.0","id":"search-1","method":"tools/call","params":{
  "name":"memory_search",
  "arguments":{
...
```json
{"jsonrpc":"2.0","id":"read-1","method":"resources/read","params":{
...
```json
{"jsonrpc":"2.0","id":"list-1","method":"tools/call","params":{
  "name":"memory_list",
...
const srv = spawn('node', ['path/to/memory-server.js'], { stdio: ['pipe','pipe','pipe'], env: process.env });
...
rpc('init', 'initialize', { protocolVersion: '2024-11-05', clientInfo: { name: 'agent', version: '1.0' } });
